...
class CarPricePredictorPreprocessor:
    ...
    def preprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_cat['Brand'] = df_cat.name.str.split(' ').apply(lambda x: x[0] if len(x) != 0 else '')
        df_cat.drop('name', axis=1, inplace=True)

        df_cat_coded = pd.DataFrame(data=self.ohe.transform(df_cat),
                                    columns=self.ohe.get_feature_names_out()).astype(int)

        df_real = df.drop(df.loc[:, cat_features_list], axis=1)

        df_real_no_na = pd.DataFrame(data=self.na_imputer.transform(df_real),
        ...
        year_poly = pd.DataFrame(poly.fit_transform(df_real_no_na['year'].values.reshape(-1, 1))).astype(int)
        ...
        df_real_no_na_poly_std = pd.DataFrame(self.normalizer.transform(df_real_no_na_poly),
